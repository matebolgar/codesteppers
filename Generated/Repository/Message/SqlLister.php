...
use CodeSteppers\Generated\Message\Listing\CountedMessages;
...
class SqlLister implements Lister
{
    ...
    public function list(Query $query): CountedMessages
    {
        try {
            ...
            return new CountedMessages(
                array_map(function ($array) {
                    return new Message((int)$array['id'], (string)$array['email'], (string)$array['subject'], (string)$array['body'], (string)$array['status'], (int)$array['numberOfAttempts'], (int)$array['sentAt'], (int)$array['createdAt']);
                }, $results),
                $count
            );
    ...
    private function getSelectQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `messages` %s %s %s LIMIT ? OFFSET ?",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,

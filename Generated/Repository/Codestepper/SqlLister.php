...
use CodeSteppers\Generated\Codestepper\Listing\CountedCodesteppers;
...
class SqlLister implements Lister
{
    ...
    public function list(Query $query): CountedCodesteppers
    {
        try {
            ...
            if (!$stmt) { 
                return new CountedCodesteppers([], 0);
            }
    ...
    private function getSelectQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `codesteppers` %s %s %s LIMIT ? OFFSET ?",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
    ...
    private function getCountQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `codesteppers` %s %s %s",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
                $query->getOrderBy()->getValue()
